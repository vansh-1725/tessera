...
function LoginForm() {
  ...
  useEffect(() => {
    if (searchParams.get("registered") === "true") setSuccess("Account created! Please sign in.");
  }, [searchParams]);
    ...
    <div style={{ background: "#060912", minHeight: "100vh", display: "flex",
      ...
      <div style={{ width: "100%", maxWidth: 440, zIndex: 10 }}>
        ...
        <div style={{ background: "#0D1526", border: "1px solid #1e2a45", borderRadius: 16, padding: 36 }}>
          <h1 style={{ color: "#F0F4FF", fontSize: 22, fontWeight: 700, marginBottom: 6 }}>Welcome back</h1>
          <p style={{ color: "#64748b", fontSize: 14, marginBottom: 28 }}>Sign in to your Tessera account.</p>

          {success && (
            <div style={{ background: "rgba(16,217,138,0.1)", border: "1px solid rgba(16,217,138,0.3)",
              borderRadius: 8, padding: "10px 14px", marginBottom: 20, color: "#10D98A", fontSize: 13 }}>
              {success}
            </div>
          )}
          {error && (
          ...
          <p style={{ textAlign: "center", color: "#64748b", fontSize: 14, marginTop: 24 }}>
            ...
            <Link href="/auth/register" style={{ color: "#2A6FDB", fontWeight: 600, textDecoration: "none" }}>
              Sign up free
            </Link>
          </p>
        </div>
