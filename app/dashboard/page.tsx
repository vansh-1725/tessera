    ...
    <div style={{ fontFamily: "DM Sans, sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)",
        ...
        {[
          ...
          <div key={s.label} style={{ background: "#0D1526",
            border: "1px solid #1e2a45", borderRadius: 12, padding: "20px 24px" }}>
            <div style={{ fontSize: 22, marginBottom: 8 }}>{s.icon}</div>
            <div style={{ color: s.color, fontSize: 24, fontWeight: 700, marginBottom: 2 }}>
              {s.value}
            </div>
      ...
      {/* Quick Actions */}
      <h2 style={{ color: "#94A3C4", fontSize: 12, fontWeight: 600,
        letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 14 }}>
        Quick Actions
      </h2>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
        {[
          { icon: "📄", title: "Upload PDF", desc: "Generate a quiz from any PDF",
            color: "#2A6FDB", href: "/upload" },
          { icon: "▶️", title: "YouTube Video", desc: "Summarise and quiz any video",
            color: "#FF4D6D", href: "/youtube" },
          { icon: "🤖", title: "AI Chat", desc: "Ask your study agent anything",
            color: "#8B5CF6", href: "/chat" },
        ].map((a) => (
          <div key={a.title} onClick={() => router.push(a.href)}
